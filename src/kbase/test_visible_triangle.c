...
int main(void) {
    printf("=== Test 1: Visible Triangle (Solid Color Fill) ===\n\n");
    ...
    /* Color buffer - 32x32 pixels (128 bytes) */
    volatile uint32_t *color = (volatile uint32_t *)((uint8_t*)cpu + 0x40);
    for (int i = 0; i < 32*32; i++) {
        color[i] = 0xDEADBEEF;  /* Initial: magenta */
    }
    ...
    vertices[0] = 0.0f; vertices[1] = 0.8f; vertices[2] = 0.0f; vertices[3] = 1.0f;
    vertices[4] = -0.8f; vertices[5] = -0.8f; vertices[6] = 0.0f; vertices[7] = 1.0f;
    vertices[8] = 0.8f; vertices[9] = -0.8f; vertices[10] = 0.0f; vertices[11] = 1.0f;
    ...
    printf("Changed pixels: %d / %d\n", changed, 32*32);
    printf("First pixel: 0x%08x\n", color[0]);
    printf("Center pixel (idx 512): 0x%08x\n", color[512]);
