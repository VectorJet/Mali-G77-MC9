...
int main(void) {
    printf("=== 3 atoms but with FRAGMENT (just check return) ===\n");
    ...
    /* Just 2 TILER + 1 FRAGMENT */
    *(uint32_t*)((uint8_t*)cpu + 0x200) = 1;
    *(uint32_t*)((uint8_t*)cpu + 0x400) = 1;

    /* FBD */
    uint32_t *fbd = (uint32_t *)((uint8_t*)cpu + 0x600);
    fbd[0x80/4 + 0] = 256;
    fbd[0x80/4 + 1] = 256;
    fbd[0x80/4 + 8] = (uint32_t)(gva + 0x700);

    uint32_t *rt = (uint32_t *)((uint8_t*)cpu + 0x700);
    ...
    printf("Submitting T->T->F (3 atoms)...\n");
    ...
    *color = 0xDEADBEEF;
    printf("Color after wait: 0x%08x\n", *color);
