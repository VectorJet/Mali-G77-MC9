...
/* The standard kbase atom - should be 72 bytes */
struct kbase_atom {
    uint64_t seq_nr;
    uint64_t jc;
    uint64_t udata[2];
...
int main(void) {
    printf("sizeof(kbase_atom) = %zu\n", sizeof(struct kbase_atom));
    printf("=== 3 atoms with VERIFIED struct ===\n");
