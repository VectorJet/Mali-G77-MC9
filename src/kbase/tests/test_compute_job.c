...
int main(void) {
    printf("=== Compute Job Test (type=3, core_req=0x002) ===\n");
    ...
    *target = 0xDEADBEEF;
    ...
    /* COMPUTE job - type=3 but core_req=0x002 (compute shader) not vertex */
    uint32_t *job = (uint32_t *)cpu;
    job[4] = (3 << 1) | (1 << 16);  /* type=3, index=1 */
    job[8] = (uint32_t)(gva + 0x100);  /* Uniforms or shared memory */
    job[12] = 0;

    printf("COMPUTE job at 0x%llx\n", (unsigned long long)gva);

    struct kbase_atom atom = {0};
    atom.jc = gva;
    atom.core_req = 0x002;  /* Compute Shader, not Vertex */
    atom.atom_number = 1;
