...
/*
 * Valhall Job Chain Test
 * 
 * Use next_job_ptr at offset 0x18 to chain multiple jobs
...
int main(void) {
    ...
    printf("=== Mali - Job Chain (next_job_ptr) ===\n\n");
    ...
    *target1 = 0xAAAAAAAA;
    *target2 = 0xBBBBBBBB;
    printf("[OK] target1 = 0xAAAAAAAA at 0x%llx\n", (unsigned long long)(gpu_va + 0x40));
    printf("[OK] target2 = 0xBBBBBBBB at 0x%llx\n", (unsigned long long)(gpu_va + 0x80));

    /* Job 1 at offset 0 */
    job = (uint32_t *)((uint8_t *)cpu_mem + 0);
    job[4] = (2 << 1) | (1 << 16);  /* WRITE_VALUE, index=1 */
    job[8] = (uint32_t)(gpu_va + 0x40);  /* target 1 */
    job[9] = 0;
    job[10] = 6;  /* IMMEDIATE32 */
    job[12] = 0x11111111;
    /* next_job_ptr at offset 0x18 -> job 2 at 0x100 */
    *(uint64_t *)((uint8_t *)cpu_mem + 0x18) = gpu_va + 0x100;

    /* Job 2 at offset 0x100 */
    job = (uint32_t *)((uint8_t *)cpu_mem + 0x100);
    job[4] = (2 << 1) | (2 << 16);  /* WRITE_VALUE, index=2 */
    job[8] = (uint32_t)(gpu_va + 0x80);  /* target 2 */
    job[9] = 0;
    job[10] = 6;
    job[12] = 0x22222222;
    /* next_job_ptr = 0 (end of chain) */
    *(uint64_t *)((uint8_t *)cpu_mem + 0x118) = 0;

    printf("[OK] Job1 at 0x%llx -> next=0x%llx\n", (unsigned long long)gpu_va, (unsigned long long)(gpu_va + 0x100));
    printf("[OK] Job2 at 0x%llx -> next=0\n", (unsigned long long)(gpu_va + 0x100));

    /* Single atom with job chain */
    atom.jc = gpu_va;  /* first job */
    atom.core_req = 0x203;
    atom.atom_number = 1;
    atom.prio = 0;

    printf("[OK] Atom: jc=0x%llx, job chain to two jobs\n", (unsigned long long)gpu_va);
    ...
    if (*target1 == 0x11111111 && *target2 == 0x22222222) {
        ...
        printf("\n*** PARTIAL: First job executed, second may not ***\n");
    }
