...
int submit_one(int fd, uint32_t core_req, int index) {
    ...
    job[8] = (uint32_t)(mem[1] + 0x100);  /* target = marker location */
    job[9] = 0;
    job[10] = 6;  /* Immediate32 */
    job[12] = 0x11110000 | index;  /* value with index */
    
    /* Marker at offset 0x100 */
    volatile uint32_t *marker = (volatile uint32_t *)((char*)cpu + 0x100);
    *marker = 0xABCD0000 | index;
    ...
    /* Read event to allow next submit */
    uint8_t ev[24];
    read(fd, ev, sizeof(ev));
    
    printf("Submit %d: ret=%d, marker was 0x%08x, now 0x%08x\n", 
           index, ret, *marker & 0xFFFF0000, *marker);
...
int main(void) {
    ...
    printf("=== Multiple submits WITH event read ===\n");
    
    submit_one(fd, 0x000, 0);
    submit_one(fd, 0x010, 1);
    submit_one(fd, 0x100, 2);
    submit_one(fd, 0x200, 3);
    submit_one(fd, 0x203, 4);
    submit_one(fd, 0x001, 5);
    submit_one(fd, 0x002, 6);
    submit_one(fd, 0x201, 7);
    submit_one(fd, 0x202, 8);
