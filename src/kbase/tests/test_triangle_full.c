...
int main(void) {
    printf("=== RENDER TRIANGLE: Full Pipeline ===\n\n");
    ...
    *color = 0x00000000;
    ...
    /* 
     * STAGE 3: TILER - Mark tiles
     */
    uint32_t *job2 = (uint32_t *)((uint8_t*)cpu + 0x400);
    job2[4] = (4 << 1) | (1 << 16);  /* TILER */
    job2[8] = (uint32_t)(gva + 0x600);  /* polygon list */
    job2[12] = 0;

    /* Polygon list at 0x600 */
    volatile uint32_t *poly = (volatile uint32_t *)((uint8_t*)cpu + 0x600);
    *poly = 0x00000001;

    /* 
     * STAGE 4: FRAGMENT - Render to color buffer
     */
    ...
    fbd[0x80/4 + 8] = (uint32_t)(gva + 0xA00);  /* RT */

    /* RT at 0xA00 - points to color buffer */
    uint32_t *rt = (uint32_t *)((uint8_t*)cpu + 0xA00);
    ...
    /* Multi-atom submission with dependencies */
    ...
    if (*color != 0x00000000) {
        ...
        printf("\nNo change - needs proper vertex/tiler setup\n");
    }
