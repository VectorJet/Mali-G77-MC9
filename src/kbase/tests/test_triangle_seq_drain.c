...
int main(void) {
    printf("=== Test: Triangle Rendering (Full Pipeline) ===\n\n");
    ...
    *color = 0xDEADBEEF;
    ...
    job0[4] = (2 << 1) | (1 << 16);
    job0[8] = (uint32_t)(gva + 0x100);  /* polygon list */
    job0[12] = 0x00000001;
    ...
    if (*color == 0xDEADBEEF) {
        printf("No change - need proper vertex/tiler data\n");
    } else if (*color == 0x00000000) {
        printf("Cleared - pipeline works but need triangle data\n");
    } else {
