...
/*
 * Valhall fragment job test
 * 
 * We know:
 * - WRITE_VALUE (type 2) works
 * - core_req 0x1 (FS) and 0x203 (CS+CF) both work for job submission
 * 
 * Now try to get actual fragment rendering working
 * 
 * Minimal fragment job format (from Panfrost):
 * - 128 byte header
 * - Payload contains framebuffer pointer (FBD)
 * 
 * For simplicity, let's verify the GPU can actually execute different
 * job types and write to different memory locations in one submit.
 */
int main(void) {
    int fd, ret;
    void *cpu_mem;
    ...
    printf("=== Mali - Multiple Jobs Test ===\n\n");
    ...
    *target2 = 0xBBBBBBBB;
    ...
    /* Two atoms - chained */
    atoms[0].jc = gpu_va;  /* first job at offset 0 */
    atoms[0].core_req = 0x203;
    atoms[0].atom_number = 1;
    atoms[0].prio = 0;

    atoms[1].jc = gpu_va + 0x100;  /* second job at offset 0x100 */
    ...
    printf("[OK] JOB_SUBMIT (2 atoms): ret=%d\n", ret);
