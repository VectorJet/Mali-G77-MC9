...
/* Fresh device each time */
int submit_fresh(uint32_t core_req, int value) {
    ...
    *marker = 0xFFFFFFFF;
    ...
    printf("value=0x%08x, marker after=0x%08x, %s\n", 
           value, *marker, (*marker == value) ? "SUCCESS" : "FAIL");
...
int main(void) {
    printf("=== Fresh device each submit ===\n");
    
    submit_fresh(0x010, 0x11111111);
    submit_fresh(0x010, 0x22222222);
    submit_fresh(0x010, 0x33333333);
    submit_fresh(0x203, 0x44444444);
    submit_fresh(0x001, 0x55555555);
