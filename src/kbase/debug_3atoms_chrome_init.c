...
int main(void) {
    printf("=== Debug: Full Chrome init sequence, then 3 atoms ===\n");

    int fd = open("/dev/mali0", O_RDWR);
    ioctl(fd, KBASE_IOCTL_VERSION_CHECK, &(uint16_t){11});
    ioctl(fd, KBASE_IOCTL_SET_FLAGS, &(uint32_t){0});

    /* Chrome calls multiple 0x19 with different values - try them in sequence */
    printf("Calling 0x19 sequence...\n");
    uint32_t vals19[] = {0x180, 0x173, 0x14d, 0x13f, 0x108, 0x111, 0x131, 0x152, 0x153, 0x161};
    for (int i = 0; i < 10; i++) {
        ioctl(fd, _IOC(_IOC_WRITE, 0x80, 0x19, 4), &vals19[i]);
    }

    /* Also try GET_GPUPROPS to see if it changes anything */
    printf("Calling GET_GPUPROPS...\n");
    uint64_t props[2] = {0, 16};
    ioctl(fd, KBASE_IOCTL_GET_GPUPROPS, props);
