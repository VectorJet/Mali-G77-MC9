...
#define KBASE_IOCTL_GET_API_VERSION   _IOC(_IOC_READ, 0x80, 0, 0)
#define KBASE_IOCTL_POST_RUN   _IOC(_IOC_WRITE, 0x80, 0x1B, 16)
...
int main(void) {
    printf("=== Debug: 3 atoms + POST_RUN ioctl ===\n");

    int fd = open("/dev/mali0", O_RDWR);
    ioctl(fd, KBASE_IOCTL_VERSION_CHECK, &(uint16_t){11});
    ioctl(fd, KBASE_IOCTL_SET_FLAGS, &(uint32_t){0});

    /* Try calling POST_RUN-like ioctl before submit */
    struct { uint64_t a, b; } post = {0, 0};
    ioctl(fd, 0x801a461b, &post);  /* Chrome's post ioctl */
