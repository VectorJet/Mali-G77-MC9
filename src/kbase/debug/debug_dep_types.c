...
int main(void) {
    printf("=== Debug: 3 atoms, try ALL pre_dep_type values ===\n");

    int fd = open("/dev/mali0", O_RDWR);
    ioctl(fd, KBASE_IOCTL_VERSION_CHECK, &(uint16_t){11});
    ioctl(fd, KBASE_IOCTL_SET_FLAGS, &(uint32_t){0});

    for (int dep_type = 0; dep_type <= 3; dep_type++) {
        printf("\n--- Testing pre_dep_type = %d ---\n", dep_type);
