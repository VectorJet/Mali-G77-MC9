...
int main(void) {
    printf("=== Debug: 3 atoms NO deps (parallel), different jobslots ===\n");
    ...
    *(uint32_t*)((uint8_t*)cpu + 0x600) = 1;
    ...
    /* 3 atoms - NO dependencies, DIFFERENT jobslots for parallelism */
    ...
    atoms[1].jobslot = 1;  /* Different jobslot */

    atoms[2].jc = gva + 0x400;
    atoms[2].core_req = 0x004;
    atoms[2].atom_number = 3;
    atoms[2].jobslot = 2;  /* Different jobslot */

    printf("Submitting 3 atoms (parallel, no deps)...\n");
