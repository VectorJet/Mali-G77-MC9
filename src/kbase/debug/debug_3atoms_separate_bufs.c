...
int main(void) {
    printf("=== Debug: 3 atoms with UNIQUE jc for each ===\n");

    int fd = open("/dev/mali0", O_RDWR);
    ioctl(fd, KBASE_IOCTL_VERSION_CHECK, &(uint16_t){11});
    ioctl(fd, KBASE_IOCTL_SET_FLAGS, &(uint32_t){0});

    /* Create 3 SEPARATE allocations like Chrome */
    ...
    void *buf2 = mmap(NULL, 2*4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, mem2[1]);
    uint64_t gva2 = mem2[1];

    uint64_t mem3[4] = {2, 2, 0, 0xF};
    ioctl(fd, KBASE_IOCTL_MEM_ALLOC, mem3);
    void *buf3 = mmap(NULL, 2*4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, mem3[1]);
    uint64_t gva3 = mem3[1];

    printf("3 buffers: 0x%llx, 0x%llx, 0x%llx\n",
           (unsigned long long)gva1, (unsigned long long)gva2, (unsigned long long)gva3);

    memset(buf1, 0, 2*4096);
    memset(buf2, 0, 2*4096);
    memset(buf3, 0, 2*4096);
