...
int main(void) {
    printf("=== Debug: 3 atoms with TIMEOUT reset ===\n");
    ...
    *(uint32_t*)((uint8_t*)cpu + 0x600) = 1;
    ...
    /* Try with proper PRIO set like real drivers */
    struct kbase_atom atoms[3] = {0};

    atoms[0].jc = gva;
    atoms[0].core_req = 0x004;
    atoms[0].atom_number = 1;
    atoms[0].prio = 0;  /* Normal priority */
    ...
    printf("Submitting 3 atoms with explicit prio=0...\n");
    ioctl(fd, KBASE_IOCTL_JOB_SUBMIT, &(struct { uint64_t addr; uint32_t nr, stride; }){
        .addr = (uint64_t)&atoms, .nr = 3, .stride = 72});

    /* Quick drain attempt */
    usleep(50000);
    int drained = read(fd, (uint8_t[24]){0}, 24);
    printf("Quick drain: %d bytes\n", drained);
