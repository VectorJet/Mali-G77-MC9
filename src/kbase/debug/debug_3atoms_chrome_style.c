...
int main(void) {
    printf("=== Debug: 3 atoms like Chrome (DIFFERENT types, seq_nr) ===\n");
    ...
    /* Create 3 DIFFERENT jobs like Chrome does */
    /* Job 0: WRITE_VALUE */
    ...
    *(uint32_t*)((uint8_t*)cpu + 0x300) = 1;
    ...
    /* 3 atoms with Chrome-like properties - DIFFERENT types */
    struct kbase_atom atoms[3] = {0};

    /* Atom 0: WV - CS+CF */
    atoms[0].jc = gva;
    atoms[0].core_req = 0x203;  /* CS + CF like Chrome atom 0 */
    ...
    atoms[1].seq_nr = 0x11;  /* Like Chrome */
    atoms[1].prio = 0;
    atoms[1].device_nr = 0;
    atoms[1].jobslot = 0;
    ...
    atoms[2].seq_nr = 0x11;  /* Like Chrome */
    atoms[2].prio = 0;
    atoms[2].device_nr = 0;
    atoms[2].jobslot = 0;
    atoms[2].pre_dep_atom[0] = 1;  /* depends on atom 1 */
    atoms[2].pre_dep_type[0] = 1;

    printf("Submitting 3 atoms (WV->T->F)...\n");
