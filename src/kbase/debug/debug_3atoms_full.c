...
int main(void) {
    printf("=== Debug: 3 atoms with EVERY field set ===\n");
    ...
    *(uint32_t*)((uint8_t*)cpu + 0x600) = 1;
    ...
    /* Create jobs */
    for (int i = 0; i < 3; i++) {
        uint32_t *job = (uint32_t *)((uint8_t*)cpu + i*0x200);
        job[4] = (4 << 1) | (1 << 16);
        job[8] = (uint32_t)(gva + 0x200 + i*0x200);
        job[12] = 0;
    }

    /* 3 atoms with FULL structure - like real drivers do */
    ...
    atoms[1].pre_dep_type[0] = 1;  /* JOB_COMPLETE */
    ...
    printf("Submitting 3 atoms (all TILER, chained)...\n");
