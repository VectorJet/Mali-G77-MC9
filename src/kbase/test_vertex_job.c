...
int main(void) {
    ...
    *target = 0xDEADBEEF;
    ...
    /* VERTEX job - type=3 (based on test_all_jobs) */
    uint32_t *job = (uint32_t *)cpu;
    job[4] = (3 << 1) | (1 << 16);  /* VERTEX type=3 */
    job[8] = (uint32_t)(gva + 0x100);  /* attribute buffer */
    job[9] = 0;
    job[10] = 0;  /* unused */
    job[11] = 0;  /* unused */
    job[12] = 0;  /* unused */

    printf("VERTEX job at 0x%llx, attr=0x%llx\n", 
           (unsigned long long)gva, (unsigned long long)(gva+0x100));

    /* core_req=0x008 (Vertex) */
