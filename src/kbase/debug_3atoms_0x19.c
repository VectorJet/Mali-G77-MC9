...
int main(void) {
    printf("=== Debug: 3 atoms + ioctl 0x19 BEFORE submit ===\n");

    int fd = open("/dev/mali0", O_RDWR);
    ioctl(fd, KBASE_IOCTL_VERSION_CHECK, &(uint16_t){11});
    ioctl(fd, KBASE_IOCTL_SET_FLAGS, &(uint32_t){0});

    /* Chrome calls 0x19 BEFORE submit - try different values */
    printf("Calling ioctl 0x19 before submit...\n");
    uint32_t test_19_values[] = {0x180, 0x173, 0x14d, 0x13f, 0x108, 0x100};
    
    for (int i = 0; i < 3; i++) {
        ioctl(fd, _IOC(_IOC_WRITE, 0x80, 0x19, 4), &test_19_values[i]);
    }
    ...
    *(uint32_t*)((uint8_t*)cpu + 0x600) = 1;
    ...
    atoms[0].jc = gva; atoms[0].core_req = 0x004; atoms[0].atom_number = 1; atoms[0].seq_nr = 0;
