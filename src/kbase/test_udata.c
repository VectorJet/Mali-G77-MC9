...
int main(void) {
    ...
    printf("=== Test: Like Chrome - udata points to target ===\n");
    ...
    *marker = 0xDEADBEEF;
    ...
    /* Atom like Chrome: udata[0] points to where completion should go */
    struct kbase_atom atom = {0};
    atom.jc = mem[1];  /* job */
    atom.udata[0] = mem[1] + 0x100;  /* like Chrome: point to marker! */
    atom.udata[1] = 0;
    atom.core_req = 0x010;
    atom.atom_number = 1;
    
    printf("jc=0x%llx, udata[0]=0x%llx, udata[1]=0x%llx\n",
    ...
    /* Also check event for udata */
    printf("event bytes: ");
    for (int i = 0; i < 24; i++) printf("%02x ", event[i]);
    printf("\n");
