...
int main(void) {
    printf("=== Simple 2-Atom: TILER + FRAGMENT ===\n\n");
    ...
    *poly = 0x00000001;
    ...
    fbd[0x80/4 + 0] = 256;
    fbd[0x80/4 + 1] = 256;
    fbd[0x80/4 + 8] = (uint32_t)(gva + 0x600);  /* RT */

    /* RT at 0x600 - points to color */
    uint32_t *rt = (uint32_t *)((uint8_t*)cpu + 0x600);
    rt[0] = (uint32_t)(gva + 0x30);
    rt[2] = 4;

    printf("TILER at 0x%llx, FRAGMENT at 0x%llx\n",
           (unsigned long long)gva, (unsigned long long)(gva+0x300));

    /* 2 atoms: TILER -> FRAGMENT */
