...
int main(void) {
    printf("=== Test: Different Jobslots ===\n\n");
    ...
    *target = 0xDEADBEEF;
    ...
    printf("Job 0 (jobslot 0) done\n");

    /* Job 2: TILER at offset 0x300 - DIFFERENT jobslot */
    ...
    atom1.atom_number = 1;
    atom1.jobslot = 1;  /* jobslot 1 - different! */

    ioctl(fd, KBASE_IOCTL_JOB_SUBMIT, &(struct { uint64_t addr; uint32_t nr, stride; }){
        .addr = (uint64_t)&atom1, .nr = 1, .stride = 72});
    usleep(50000);
    read(fd, (uint8_t[24]){0}, 24);
    printf("Job 1 (jobslot 1) done\n");

    printf("poly at 0x200=0x%08x, poly at 0x400=0x%08x\n",
