...
int main(void) {
    printf("=== Debug: 3 atoms + 0x1b with pointer structure ===\n");
    ...
    *(uint32_t*)((uint8_t*)cpu + 0x600) = 1;
    ...
    /* NOW call 0x1b - Chrome does this AFTER submit */
    printf("Calling 0x1b after submit...\n");
    struct { uint64_t ptr; uint32_t val; } post = {gva, 0x148f};
    ioctl(fd, _IOC(_IOC_WRITE, 0x80, 0x1B, 16), &post);
