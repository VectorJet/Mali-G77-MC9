...
int main(void) {
    printf("=== Try different MEM_ALLOC flags ===\n");
    ...
    printf("Test 1: flags=0xF (default)\n");
    {
    ...
    printf("Test 2: flags=0x3 (CPU only)\n");
    {
        uint64_t mem[4] = {2, 2, 0, 0x3};
        ioctl(fd, KBASE_IOCTL_MEM_ALLOC, mem);
        void *cpu = mmap(NULL, 8192, 3, 1, fd, mem[1]);
        if (cpu == MAP_FAILED) { printf("  mmap failed\n"); }
        else {
            memset(cpu, 0, 8192);
            uint32_t *job = (uint32_t *)cpu;
            job[4] = (2 << 1) | (1 << 16);
    ...
    printf("Test 3: flags=0xC (GPU only)\n");
    {
        uint64_t mem[4] = {2, 2, 0, 0xC};
        ioctl(fd, KBASE_IOCTL_MEM_ALLOC, mem);
        printf("  gpu_va=0x%llx\n", (unsigned long long)mem[1]);
        /* Can't mmap GPU-only */
    }
    
    printf("Test 4: different commit pages (mem[0])\n");
    {
        uint64_t mem[4] = {4, 4, 0, 0xF};  /* 4 pages committed */
        ioctl(fd, KBASE_IOCTL_MEM_ALLOC, mem);
