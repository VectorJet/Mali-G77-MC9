...
int main(void) {
    ...
    printf("=== Use test_multi pattern: index in both values ===\n");
    
    /* Same pattern as test_multi: index in both marker and value */
    for (int i = 0; i < 3; i++) {
        ...
        job[9] = 0;
        job[10] = 6;
        job[12] = 0x11110000 | i;  /* Same pattern as test_multi */
        
        volatile uint32_t *marker = (volatile uint32_t *)((char*)cpu + 0x100);
        *marker = 0xABCD0000 | i;  /* Same pattern as test_multi */
        ...
        printf("i=%d: marker was 0x%08x, now 0x%08x\n", 
               i, 0xABCD0000 | i, *marker);
