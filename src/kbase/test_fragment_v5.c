...
int main(void) {
    printf("=== Test Multi-Atom: WRITE_VALUE + FRAGMENT v2 ===\n\n");
    ...
    uint64_t mem1[4] = {4, 4, 0, 0xF};
    ioctl(fd, KBASE_IOCTL_MEM_ALLOC, mem1);
    void *buf1 = mmap(NULL, 4*4096, PROT_READ|PROT_WRITE, MAP_SHARED, fd, mem1[1]);
    ...
    memset(buf1, 0, 4*4096);
    memset(buf2, 0, 8*4096);

    /* Color buffer in buf2 at offset 0x100 to avoid conflict with job */
