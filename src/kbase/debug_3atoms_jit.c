...
/* KBASE_IOCTL_MEM_JIT_INIT - NR 14 */
#define KBASE_IOCTL_MEM_JIT_INIT  _IOC(_IOC_WRITE, 0x80, 14, 16)

int main(void) {
    printf("=== Debug: Try MEM_JIT_INIT before 3 atoms ===\n");

    int fd = open("/dev/mali0", O_RDWR);
    ioctl(fd, KBASE_IOCTL_VERSION_CHECK, &(uint16_t){11});
    ioctl(fd, KBASE_IOCTL_SET_FLAGS, &(uint32_t){0});

    /* Try MEM_JIT_INIT with proper structure */
    struct { uint32_t id; uint32_t val; uint64_t addr; uint64_t size; } jit = {0};
    int ret = ioctl(fd, KBASE_IOCTL_MEM_JIT_INIT, &jit);
    printf("MEM_JIT_INIT ret = %d\n", ret);

    /* Try another 0x19 after */
    uint32_t cfg = 0x180;
