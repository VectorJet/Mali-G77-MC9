...
struct v9_framebuffer *v9_framebuffer_create(struct kbase_dev *dev, uint32_t width, uint32_t height) {
    ...
    /* Keep writable job data separate from the executable shader allocation. */
    ...
    fb->exec_bo = kbase_bo_alloc(dev, 4096,
                                KBASE_BO_PROT_READ | KBASE_BO_PROT_WRITE | KBASE_BO_PROT_EXEC);
    if (!fb->exec_bo) {
    ...
    fb->sampleloc_gpu            = base_gva + 0xB100;
    ...
    uint32_t *bl = (uint32_t *)(base_cpu + (fb->blend_gpu - base_gva));
    bl[0] = (1u << 9);
    ...
    uint32_t *ls = (uint32_t *)(base_cpu + (fb->tls_gpu - base_gva));
    memset(ls, 0, 32);
    ls[0] = 0;
    ls[1] = 0x80000000u;
    ...
    uint32_t *zs = (uint32_t *)(base_cpu + (fb->depth_gpu - base_gva));
    zs[0] = (7u << 0) | (7u << 4) | (7u << 16);
    zs[4] = (1u << 22) | (7u << 29);

    /* Initialize Position buffer (NDC full-screen triangle) */
    float *pos = (float *)(base_cpu + (fb->pos_gpu - base_gva));
    ...
    uint16_t *idx = (uint16_t *)(base_cpu + (fb->idx_gpu - base_gva));
    idx[0] = 0; idx[1] = 1; idx[2] = 2;

    /* Initialize SHADER_PROGRAM descriptor */
    uint32_t *sp = (uint32_t *)(base_cpu + (fb->sp_gpu - base_gva));
    sp[0] = (8u << 0) | (2u << 4) | (1u << 8) | (1u << 28) | (2u << 30);
    sp[1] = 0;
    *(uint64_t *)(sp + 2) = fb->isa_gpu;

    /* Initialize TILER_HEAP descriptor */
    uint32_t *th = (uint32_t *)(base_cpu + (fb->tiler_heap_desc_gpu - base_gva));
    th[0] = (9u << 0) | (2u << 4) | (0u << 8);
    th[1] = 0x40000;
    ...
    *(uint64_t *)(th + 6) = fb->tiler_heap_backing_gpu + 0x40000;
    ...
    uint32_t *tc = (uint32_t *)(base_cpu + (fb->tiler_ctx_gpu - base_gva));
    ...
    *(uint64_t *)(tc + 6) = fb->tiler_heap_desc_gpu;
    ...
    uint32_t *rt0 = (uint32_t *)(base_cpu + (fb->rt0_gpu - base_gva));
    rt0[0] = (1 << 26);
    uint32_t swizzle_rgba = (0 << 0) | (1 << 3) | (2 << 6) | (3 << 9);
    rt0[1] = (1 << 0) | (19 << 3) | (2 << 8) | (1 << 15) | (swizzle_rgba << 16) | (1u << 31);
    *(uint64_t *)(base_cpu + (fb->rt0_gpu - base_gva) + 0x20) = color_gpu;
    *(uint32_t *)(base_cpu + (fb->rt0_gpu - base_gva) + 0x28) = width * 4;
    uint32_t clear_val = 0xFF0000FF;
    *(uint32_t *)(base_cpu + (fb->rt0_gpu - base_gva) + 0x30) = clear_val;
    *(uint32_t *)(base_cpu + (fb->rt0_gpu - base_gva) + 0x34) = clear_val;
    *(uint32_t *)(base_cpu + (fb->rt0_gpu - base_gva) + 0x38) = clear_val;
    *(uint32_t *)(base_cpu + (fb->rt0_gpu - base_gva) + 0x3C) = clear_val;

    /* Initialize Sample Location Table */
    uint16_t *sl = (uint16_t *)(base_cpu + (fb->sampleloc_gpu - base_gva));
    memset(sl, 0, 192);
    sl[0] = 128; sl[1] = 128;
    for (int i = 1; i < 32; i++) { sl[i*2] = 0; sl[i*2+1] = 256; }
    sl[64] = 128; sl[65] = 128;
    *(uint64_t *)(base_cpu + 0x6000 + 0x10) = fb->sampleloc_gpu;
    ...
    *(uint64_t *)(params + 6) = fb->tiler_ctx_gpu;
    ...
    /* Initialize Pre Frame 0 DCD */
    uint32_t *dcd = (uint32_t *)(base_cpu + (fb->dcd_gpu - base_gva));
    memset(dcd, 0, 3 * 128);
    ...
    *(uint64_t *)(dcd + 30) = 0;
    ...
    uint32_t *vt = (uint32_t *)(base_cpu + (fb->tiler_job_gpu - base_gva));
    vt[4] = (1u << 0) | (7u << 1);
    ...
    *(uint64_t *)(se + 14) = 0;
    ...
    uint32_t *fl = (uint32_t *)(base_cpu + (fb->flush_jc_gpu - base_gva));
    fl[4] = (3u << 1);
    fl[8] = 0xFFFFFFFFu; fl[9] = 0xFFFFFFFFu;

    /* Build Fragment JC (Type 9) */
    uint32_t *fj = (uint32_t *)(base_cpu + (fb->frag_jc_gpu - base_gva));
    fj[4] = (1u << 0) | (9u << 1);
...
int v9_render_triangle(struct v9_framebuffer *fb) {
    ...
    uint32_t *vt = (uint32_t *)(base_cpu + (fb->tiler_job_gpu - fb->mem_bo->gpu));
    memset(vt, 0, 32);
    vt[4] = (1u << 0) | (7u << 1);
    *(uint64_t *)(vt + 6) = 0;
    ...
    /* Dump polygon list and heap state right after TILER_JOB, before reinit clears them */
    {
        ...
        uint64_t *th_p = (uint64_t *)(base_cpu + (fb->tiler_heap_desc_gpu - fb->mem_bo->gpu));
        ...
        uint64_t *tc_p = (uint64_t *)(base_cpu + (fb->tiler_ctx_gpu - fb->mem_bo->gpu));
        ...
        if (heap_sz > 0 && heap_sz <= 4096) {
            ...
            uint64_t *hp = (uint64_t *)(base_cpu + (fb->tiler_heap_backing_gpu - fb->mem_bo->gpu));
            for (int i = 0; i < 4; i++) {
    ...
    uint32_t *th = (uint32_t *)(base_cpu + (fb->tiler_heap_desc_gpu - fb->mem_bo->gpu));
    *(uint64_t *)(th + 4) = fb->tiler_heap_backing_gpu; /* Fragment reads from the heap base. */

    uint32_t *fj = (uint32_t *)(base_cpu + (fb->frag_jc_gpu - fb->mem_bo->gpu));
    memset(fj, 0, 32);
    fj[4] = (1u << 0) | (9u << 1);
...
#define DUMP_BLOCK(label, off, words) do { \
    ...
    for (unsigned _i = 0; _i < (words); _i += 2) { \
        ...
        DUMP_BLOCK("DCD (pre0)",      fb->dcd_gpu - fb->mem_bo->gpu, 16);
        DUMP_BLOCK("SHADER_PROGRAM",  fb->sp_gpu - fb->mem_bo->gpu, 4);
        DUMP_BLOCK("Tiler Heap Desc", fb->tiler_heap_desc_gpu - fb->mem_bo->gpu, 4);
        DUMP_BLOCK("Tiler Context",   fb->tiler_ctx_gpu - fb->mem_bo->gpu, 24);
        DUMP_BLOCK("Blend",           fb->blend_gpu - fb->mem_bo->gpu, 2);
        DUMP_BLOCK("Depth/Stencil",   fb->depth_gpu - fb->mem_bo->gpu, 4);
        DUMP_BLOCK("TLS",             fb->tls_gpu - fb->mem_bo->gpu, 4);
        DUMP_BLOCK("Fragment JC",     fb->frag_jc_gpu - fb->mem_bo->gpu, 8);
#undef DUMP_BLOCK
    }
...
#define CHECK_PTR(label, addr, exp_in) do { \
    uint64_t _a = (addr); \
    int _ok = ((exp_in) == 1 && _a >= mem_base && _a < mem_end) || \
              ((exp_in) == 2 && _a >= exec_base && _a < exec_end); \
    printf("  %-20s 0x%016llx  %s\n", (label), (unsigned long long)_a, \
        ...
        uint64_t heap_top = *(uint64_t *)(th + 6);
...
#undef CHECK_PTR
    ...
    ret = kbase_submit_job(dev, fb->frag_jc_gpu, KBASE_QUEUE_REQ_FRAGMENT, 2, 0, 0);
    if (ret < 0) return ret;

    /* The working replay usually receives no Fragment event before this flush. */
    ret = kbase_wait_event_timeout(dev, &atom_nr, &event_code, 200);
    if (ret == 0) {
        printf("v9_render_triangle: Atom 2 (Fragment) event_code=0x%x atom_nr=%u\n",
               event_code, atom_nr);
        if (atom_nr == 2 && event_code != 0x1) return -EIO;
    } else if (ret != -EAGAIN) {
        return ret;
    }

    /* 4. Submit Atom 3: Post-Flush */
    uint32_t *fl = (uint32_t *)(base_cpu + (fb->flush_jc_gpu - fb->mem_bo->gpu));
    memset(fl, 0, 64);
    fl[4] = (3u << 1);
    fl[8] = 0xFFFFFFFFu; fl[9] = 0xFFFFFFFFu;
    ...
    ret = kbase_wait_event_timeout(dev, &atom_nr, &event_code, 200);
    if (ret == 0) {
        printf("v9_render_triangle: Atom 1 (Post-Flush) event_code=0x%x atom_nr=%u\n",
               event_code, atom_nr);
        if (atom_nr == 1 && event_code != 0x1) return -EIO;
    } else if (ret != -EAGAIN) {
        return ret;
    }
