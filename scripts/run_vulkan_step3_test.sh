...
echo "[1/3] Pushing Vulkan Step 3 sources to device..."
scp -P ${PORT} src/driver/kbase_winsys.h src/driver/kbase_winsys.c \
              ...
              src/vulkan/test_panvk_v9.c ${REMOTE}:${TARGET_DIR}/

echo "[2/3] Compiling test_panvk_v9 on device..."
ssh -p ${PORT} ${REMOTE} "cd ${TARGET_DIR} && \
    gcc -Wall -O2 -o test_panvk_v9 kbase_winsys.c pan_kmod_kbase.c v9_cmd_stream.c panvk_v9_entrypoints.c test_panvk_v9.c"

echo "[3/3] Running Vulkan Step 3 test_panvk_v9 as root..."
ssh -p ${PORT} ${REMOTE} "su -c '${TARGET_DIR}/test_panvk_v9'"
